...
def _browser_page(
    ...
    return (
        ...
        f"<title>{_html.escape(title)} — OpenHarness</title><style>"
        ...
        "</style></head><body>"
        '<div class="card"><div class="mark"><i></i>OpenHarness</div>'
        f"{icon}<h1>{_html.escape(title)}</h1><p>{_html.escape(detail)}</p>{err}</div>"
        '<div class="foot">Served locally by OpenHarness on your Mac</div>'
        "</body></html>"
    )
...
_CLOUD_DISABLED_ERROR = (
    "OpenHarness does not use a cloud account. Use manual credentials or local OAuth instead."
)
...
def create_app(manager: SessionManager) -> FastAPI:
        ...
        try:
            if body.get("git_url"):
                ...
                return {
                    "ok": False,
                    "disabled": True,
                    "error": "cloud gallery is disabled; install personas from a folder, Git URL, or zip archive",
                }
            else:
                return {
                    "ok": False,
    ...
    def cloud_gallery_detail(slug: str) -> dict[str, Any]:
        """Cloud gallery is not part of the OpenHarness no-login product."""
        return {
            "ok": False,
            "disabled": True,
            "error": "cloud gallery is disabled",
        }

    @app.get("/v1/cloud/gallery")
    def cloud_gallery() -> dict[str, Any]:
        """Cloud gallery is disabled; local persona installation stays available."""
        return {
            "ok": False,
            "disabled": True,
            "error": "cloud gallery is disabled",
    ...
    async def mcp_oauth_callback(code: str = "", state: str = "", error: str = "") -> Any:
        ...
        if error:
            return HTMLResponse(
                _browser_page(
                    "Sign-in failed",
                    "The service reported an error. Return to OpenHarness and try again.",
                    ok=False,
                    error=error,
                ),
                status_code=400,
        ...
        if not code or not mcp_oauth.deliver_callback(code, state or None):
            return HTMLResponse(
                _browser_page(
                    ...
                    "The sign-in may have timed out. Return to OpenHarness and start it again.",
                    ok=False,
                ),
                status_code=400,
            )
        return HTMLResponse(
            _browser_page(
                "Connected",
                "Sign-in complete. You can close this tab and return to OpenHarness.",
                ok=True,
            )
        )
    ...
    async def slack_status() -> dict[str, Any]:
        """Slack health, three layers: relay socket / local policy / per-team tokens."""
    ...
    async def github_status() -> dict[str, Any]:
        """GitHub health: relay socket / local policy / per-installation tokens."""
    ...
    # -- Disabled cloud-account compatibility routes --------------------------
    ...
    def cloud_telemetry(body: dict) -> dict[str, Any]:
        """Cloud telemetry is disabled in OpenHarness."""
        return {"ok": False, "telemetry_enabled": False, "disabled": True}

    @app.post("/v1/cloud/login")
    def cloud_login() -> dict[str, Any]:
        """Cloud account sign-in is not supported by OpenHarness."""
        return {"ok": False, "disabled": True, "error": _CLOUD_DISABLED_ERROR}

    @app.post("/v1/cloud/logout")
    def cloud_logout() -> dict[str, Any]:
        """No-op: there is no OpenHarness cloud account session."""
        return {
            "ok": True,
            "signed_in": False,
            "account": "",
    ...
    async def connector_connect_managed(name: str, body: Optional[dict] = None) -> dict[str, Any]:
        ...
        return {
            "ok": False,
            "disabled": True,
            "error": "cloud-managed connector login is disabled; connect manually or use local MCP OAuth",
        }
    ...
    async def managed_oauth_callback(request: Request) -> Any:
        ...
        return HTMLResponse(
            _browser_page(
                "Cloud connector login disabled",
                "OpenHarness does not accept cloud-managed OAuth callbacks.",
                ok=False,
            ),
            status_code=400,
        )
    ...
    @app.get("/v1/settings/haas-delegation")
    def settings_get_haas_delegation() -> dict[str, Any]:
        return manager.get_haas_delegation_settings()

    @app.post("/v1/settings/haas-delegation")
    def settings_set_haas_delegation(body: dict) -> dict[str, Any]:
        return manager.set_haas_delegation_settings(body or {})
