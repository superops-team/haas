    ...
    return ProviderDescriptor(
        ...
        fields=[
            ...
            ProviderField(
                ...
                default=base_url,
                placeholder=base_url,
                help=endpoint_help or f"Prefilled with {title}'s official Responses endpoint.",
            ),
        ],
        haas_api_type="responses",
...
DESCRIPTORS: list[ProviderDescriptor] = [
    ...
    # Ark provider identities stay separate: BytePlus, Volcengine's standard data plane,
    # and Volcengine Agent Plan use different regions, endpoints, credentials, and model
    # catalogs. Combining them would let one provider profile route a model to the wrong
    # service.
    _responses_compat(
        "ark",
        "BytePlus Ark",
    ...
    _compat(
        ...
        recommended_model="doubao-seed-2-1-turbo-260628",
        env_key="VOLCENGINE_ARK_API_KEY",
        haas_api_type="responses",
        endpoint_help=(
            "Volcengine Ark's standard China (Beijing) data-plane endpoint. "
            "This provider is separate from Ark Agent Plan."
        ),
    ),
    _compat(
        "zai",
