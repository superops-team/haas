...
def test_verify_volcengine_ark_uses_standard_data_plane_models_probe(monkeypatch):
    cap: dict = {}
    _patch_get(monkeypatch, status=200, capture=cap)

    assert verify_provider_key("volcengine-ark", api_key="ark-key") == {"ok": True}
    assert cap["url"] == "https://ark.cn-beijing.volces.com/api/v3/models"
    assert cap["headers"]["Authorization"] == "Bearer ark-key"
