...
def test_provider_env_source_survives_remove_without_store_profile(tmp_path, monkeypatch):
    """Env-backed providers are runnable, but not locally saved keys the UI can remove."""
    monkeypatch.setenv("OPENAI_API_KEY", "sk-env-openai-placeholder")
    monkeypatch.setenv("ARK_API_KEY", "ark-env-placeholder")
    client = _client(tmp_path, [])

    prov = {p["name"]: p for p in client.get("/v1/providers").json()}
    assert prov["openai"]["configured"] is True
    assert prov["openai"]["source"] == "env"
    assert prov["openai"]["key_set_at"] is None
    assert prov["ark"]["configured"] is True
    assert prov["ark"]["source"] == "env"
    assert prov["ark"]["key_set_at"] is None

    assert client.delete("/v1/providers/openai").json()["ok"]
    assert client.delete("/v1/providers/ark").json()["ok"]
    prov = {p["name"]: p for p in client.get("/v1/providers").json()}
    assert prov["openai"]["configured"] is True
    assert prov["openai"]["source"] == "env"
    assert prov["ark"]["configured"] is True
