...
test("key form: Test verifies, saves, and returns to the gallery with the ✓", async ({
  ...
  await expect(page.getByRole("heading", { name: "Welcome to OpenHarness" })).toBeVisible();
...
test("tools page explains local connector setup and continues cleanly", async ({
  page,
}) => {
  await openOnboarding(page);
  await page.getByTestId("ob-continue").click();
  ...
  await expect(page.getByText("Connectors are local-first")).toBeVisible();
  await expect(page.getByText("OpenHarness does not require a product account.")).toBeVisible();
