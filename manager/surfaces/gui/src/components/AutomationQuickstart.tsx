...
// picking a card expands the configure card below the grid: connect rows, channel-by-name,
// day × time, and the §25 consent line for write recipes.
...
export function AutomationQuickstart({
  ...
  // is being configured so manual connector setup is reflected.
              ...
              <div key={name} className="border-b border-line last:border-b-0">
                <div className="flex items-center gap-3 py-2.5">
                  ...
                  {c?.connected ? (
                    ...
                    <span className="text-[12px] text-faint" data-testid={`ob-connect-${name}`}>
                      {t("automations.configure_in_connectors")}
                    </span>
                  )}
                </div>
              </div>
