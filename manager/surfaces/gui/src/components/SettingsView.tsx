...
import {
  ...
  setContextBar,
  setOnboarded,
  setHaasDelegationSettings,
  setPdfSettings,
  setScratchBase,
  setSessionsPeek,
  setWorkspaceTrusted,
  type CompactionSettings,
  type HaasDelegationPatch,
  type HaasDelegationSettings,
  type ModelSettings,
  type PdfSettings,
...
const SET_TABS: {
  ...
  { key: "execution", labelKey: "settings.tab.execution", icon: "shield" },
...
function ExecutionSection() {
  ...
  return (
    <section>
      <PanelHead title={t("settings.execution_title")} sub={t("settings.execution_sub")} />
      <HaasDelegationCard />
    </section>
  );
}
...
function HaasDelegationCard() {
  const { t } = useTranslation();
  const [settings, setSettings] = useState<HaasDelegationSettings | null>(null);
  ...
  const refresh = () =>
    getHaasDelegationSettings()
      .then((next) => {
        setSettings(next);
        setDraft((old) => ({
          base_url: old.base_url || next.base_url || "",
          api_token: "",
          user_id: old.user_id || next.user_id || "",
          harness_id: old.harness_id || next.harness_id || "",
          image: old.image || next.image || "",
          image_digest: old.image_digest || next.image_digest || "",
  ...
  const save = async (patch: HaasDelegationPatch) => {
    setMsg(null);
    try {
      const next = await setHaasDelegationSettings(patch);
      setSettings(next);
      setDraft((old) => ({ ...old, api_token: "", image_digest: next.image_digest || old.image_digest }));
      setMsg(next.ok === false ? next.error || t("settings.haas_save_error") : t("settings.haas_saved"));
    } catch {
      setMsg(t("settings.haas_save_error"));
    }
  };

  if (!settings) return null;
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="haas-delegation-card">
      <div className="flex items-start gap-3">
        ...
        <label className="flex items-center gap-2 text-[13px] text-ink">
          <input
            ...
            data-testid="haas-enabled"
            onChange={(e) => void save({ enabled: e.target.checked })}
          />
          {settings.enabled ? t("settings.haas_enabled") : t("settings.haas_disabled")}
        </label>
      </div>

      <label className="flex items-start gap-2 text-[13px] text-ink mt-4">
        <input
          type="checkbox"
          className="mt-0.5"
          checked={draft.local_autostart}
          data-testid="haas-local-autostart"
          onChange={(e) => setDraft((d) => ({ ...d, local_autostart: e.target.checked }))}
        />
        <span>
          <span className="block font-medium">{t("settings.haas_local_autostart")}</span>
      ...
      <label className="flex items-start gap-2 text-[13px] text-ink mt-3">
        <input
          type="checkbox"
          className="mt-0.5"
          checked={draft.allow_unpinned_local_image}
          data-testid="haas-allow-unpinned-local-image"
          onChange={(e) =>
            setDraft((d) => ({ ...d, allow_unpinned_local_image: e.target.checked }))
          }
        />
        <span>
          <span className="block font-medium">{t("settings.haas_allow_local_image")}</span>
          <span className={FIELD_HELP}>{t("settings.haas_allow_local_image_help")}</span>
        </span>
      </label>

      <div className="grid grid-cols-2 gap-3 mt-4">
        <label className="min-w-0">
          <span className={FIELD_LABEL}>{t("settings.haas_base_url")}</span>
          <input
            className={INPUT + " mt-1.5 w-full"}
            value={draft.base_url}
            data-testid="haas-base-url"
            onChange={(e) => setDraft((d) => ({ ...d, base_url: e.target.value }))}
          />
        </label>
        <label className="min-w-0">
          <span className={FIELD_LABEL}>{t("settings.haas_user_id")}</span>
          <input
            className={INPUT + " mt-1.5 w-full"}
            value={draft.user_id}
            data-testid="haas-user-id"
            onChange={(e) => setDraft((d) => ({ ...d, user_id: e.target.value }))}
          />
        </label>
        <label className="min-w-0">
          <span className={FIELD_LABEL}>{t("settings.haas_harness_id")}</span>
          <input
            className={INPUT + " mt-1.5 w-full"}
            value={draft.harness_id}
            data-testid="haas-harness-id"
            onChange={(e) => setDraft((d) => ({ ...d, harness_id: e.target.value }))}
          />
        </label>
        <label className="min-w-0">
          <span className={FIELD_LABEL}>{t("settings.haas_image")}</span>
          <input
            className={INPUT + " mt-1.5 w-full"}
            value={draft.image}
            data-testid="haas-image"
            onChange={(e) => setDraft((d) => ({ ...d, image: e.target.value }))}
          />
        </label>
        <label className="min-w-0">
          <span className={FIELD_LABEL}>{t("settings.haas_image_digest")}</span>
          <input
            className={INPUT + " mt-1.5 w-full"}
            value={draft.image_digest}
            data-testid="haas-image-digest"
            onChange={(e) => setDraft((d) => ({ ...d, image_digest: e.target.value }))}
          />
        </label>
        <label className="min-w-0">
          <span className={FIELD_LABEL}>{t("settings.haas_api_token")}</span>
          <input
            className={INPUT + " mt-1.5 w-full"}
            type="password"
            value={draft.api_token}
            placeholder={settings.has_api_token ? t("settings.haas_token_saved") : ""}
            data-testid="haas-api-token"
      ...
      <label className="block mt-3">
        <span className={FIELD_LABEL}>{t("settings.haas_triggers")}</span>
        <input
          className={INPUT + " mt-1.5 w-full"}
          value={draft.trigger_keywords}
          data-testid="haas-trigger-keywords"
      ...
      <div className="flex items-center gap-2 mt-4">
        <button
          ...
          data-testid="haas-save"
          onClick={() =>
            void save({
              base_url: draft.base_url,
              api_token: draft.api_token || undefined,
        ...
        >
          {t("settings.haas_save")}
        </button>
        {settings.has_api_token && (
          <button
            className={BTN_BORDERED}
            data-testid="haas-clear-token"
            onClick={() => void save({ clear_api_token: true })}
          >
            {t("settings.haas_clear_token")}
          </button>
        )}
