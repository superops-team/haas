  ...
  return (
    <div>
      <ProviderForm
        ...
        tp="set"
        footer={
          ps.info?.source === "store" && ps.credentialed ? (
            <button
      ...
      {c.managed && !c.mcp && !manualOnly && (
        <div className="space-y-2" data-testid="managed-connect">
          {c.managed_paused ? (
            ...
            <div className="text-[12px] text-faint" data-testid="managed-disabled">
              {t("manage.one_click_disabled")}
            </div>
          )}
          {!c.managed_paused && (
            <div className="text-[12px] text-faint">{t("manage.or_connect_manually")}</div>
