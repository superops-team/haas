...
import { FOOT, GRP, GRP_H, ROW, TAG_ACCENT, TAG_WARN, XBTN } from "./ui";
...
// OpenHarness has no product cloud account, so cloud-managed Google OAuth is
// unavailable here until a local OAuth/manual token flow is added.

export function CalendarDetail({ c, cloud: _cloud, slack: _slack, onChanged }: DetailProps) {
  ...
  return (
    <div data-testid="gcal-detail">
      <div className="flex items-center gap-3.5 mb-5">
        ...
        <button
          className="rounded-full border border-line px-4 py-1.5 text-[13px] text-faint shrink-0"
          data-testid="add-account-btn"
          disabled
          title={t("calendar.local_oauth_unavailable")}
        >
          {t("calendar.add_account_local")}
        </button>
      </div>
