...
export interface Connector {
  ...
  tools: ConnectorTool[];
  managed: boolean; // legacy cloud-managed OAuth flag; OpenHarness UI keeps it disabled
...
// --- Cloud account compatibility endpoints (disabled in OpenHarness) ---
...
/** Compatibility no-op: product cloud telemetry is disabled in OpenHarness. */
...
/** Compatibility poller for older call-sites; OpenHarness never reaches signed-in. */
...
export interface ModelSettings {
  ...
  haas_delegation?: HaasDelegationSettings;
}

export interface HaasDelegationSettings {
  ok?: boolean;
  enabled: boolean;
  base_url: string;
  user_id: string;
  ...
  image_digest?: string;
  strategy: string;
  require_trusted_workspace: boolean;
  agent_allowlist: string[];
  trigger_keywords: string[];
  idle_ttl_seconds: number;
  max_container_lifetime_seconds: number;
...
export type HaasDelegationPatch = Partial<
  Omit<
    HaasDelegationSettings,
    "ok" | "image_digest_configured" | "local_status" | "has_api_token" | "error"
  >
> & {
  api_token?: string;
...
export async function getHaasDelegationSettings(): Promise<HaasDelegationSettings> {
  const res = await fetch(`${httpBase()}/v1/settings/haas-delegation`);
  return res.json();
}

export async function setHaasDelegationSettings(
  patch: HaasDelegationPatch,
): Promise<HaasDelegationSettings> {
  const res = await fetch(`${httpBase()}/v1/settings/haas-delegation`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(patch),
...
// Legacy compatibility event for removed cloud-account flows.
...
// bot is a member (Slack API constraint); is_member=false -> "invite @OpenHarness" hint.
...
// local policy / per-installation token health (+ missed-event counts).
...
/** Slack health, three honest layers: relay socket / local policy / per-team tokens. */
