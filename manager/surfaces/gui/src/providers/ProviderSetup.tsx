...
export const KEY_HELP: Record<string, { url: string; label: string }> = {
  ...
  "volcengine-ark": { url: "https://console.volcengine.com/ark/region:ark+cn-beijing/apiKey", label: "console.volcengine.com" },
...
export function useProviderSetup(opts?: { onSaved?: () => void }): ProviderSetupState {
  ...
  const credentialed = !!info?.configured && !!info?.needs_key && info.source !== "env";
  ...
  const statusFor = (p: ProviderInfo, o?: { lastUsed?: boolean }) => {
    ...
    if (p.configured && p.needs_key) {
      if (p.source === "env") {
        return <span className="block text-[12px] text-ok font-medium truncate">{t("provider.env_configured")}</span>;
      }
