...
import { appendBoundedActivityText, finalizeCurrentHaasTurn, insertReplayedHaasTool, projectToolActivity } from "./activity";
...
describe("semantic activity projection", () => {
  it("maps a completed command to a quiet successful activity without machine copy", () => {
    expect(projectToolActivity(tool({ invocationId: "inv_1", commandPreview: "pytest -q", workingDirectory: "/workspace", evidenceRef: "evd_1", evidenceExpiresAtMs: 1789264000000 }))).toMatchObject({
      id: "call_1",
      kind: "command",
      status: "succeeded",
      title: "Ran a command",
  ...
  it("limits the normal activity preview to five lines and reports omitted lines", () => {
    const activity = projectToolActivity(
      tool({ outputPreview: ["one", "two", "three", "four", "five", "six", "seven"].join("\n") }),
    );

    expect(activity.preview).toBe(["one", "two", "three", "four", "five"].join("\n"));
  ...
  it("bounds accumulated progress while retaining both the beginning and latest detail", () => {
    const value = appendBoundedActivityText("start:" + "a".repeat(3000), "end:" + "z".repeat(3000));
  ...
  it("inserts replayed activity before the persisted HaaS final answer", () => {
    const answer: Item = { kind: "assistant", text: "Finished", source: "haas" };
    const activity = tool();
    const result = insertReplayedHaasTool(
      [{ kind: "user", text: "Do the work" }, answer],
      activity,
    );
    expect(result.map((item) => item.kind)).toEqual(["user", "tool", "assistant"]);
  });
  ...
  it("does not duplicate a persisted activity when its WebSocket lifecycle is replayed", () => {
    ...
    const result = insertReplayedHaasTool(
      [{ kind: "user", text: "Do the work" }, persisted, { kind: "assistant", text: "Finished", source: "haas" }],
      replayedStart,
    );

    expect(result.filter((item) => item.kind === "tool" && item.id === "call_1")).toHaveLength(1);
    expect(result[1]).toEqual(persisted);
  });

  it("assigns the outcome to only the current HaaS turn and terminates dangling tools", () => {
    const old = tool({ id: "old", status: "completed", taskOutcome: { phase: "completed" } });
    ...
    const result = finalizeCurrentHaasTurn(
      [old, { kind: "assistant", text: "old answer", source: "haas" }, { kind: "user", text: "next" }, current, pending, waiting, completed],
    ...
    expect((result[3] as ToolItem).safeReason).toBe("Tool ended without a terminal event");
    expect((result[3] as ToolItem).safeReason).not.toContain("Adapter disconnected");
  });
});
