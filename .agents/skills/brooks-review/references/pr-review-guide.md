...
| > 300 lines | Split the change into coherent chunks and run the full process on every chunk; if exhaustive review cannot finish, report the gate as partial, never passed |

For PRs > 500 lines, assess whether the size reflects unrelated responsibilities. Size
alone is not a finding; flag Change Propagation only when the change actually couples
unrelated concepts.
