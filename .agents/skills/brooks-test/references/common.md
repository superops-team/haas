...
Keep review evidence in the final delivery note or PR/MR description. Do not create a
repository history ledger or temporary review report.
...
If `.brooks-lint.yaml` contains a `custom_risks` map, report that this repository-local
PR-review bundle does not support custom risks and ignore the map. Do not load missing
global skill files or silently reinterpret custom rules.
...
Write a concrete, localized remedy for every finding. This repository-local skill does
not depend on separate global remedy or sweep modes. Apply fixes only when the user or
the governing repository workflow requests implementation after the review.
...
| `test-decay-risks.md` | Before any test quality review |
