...
Status: option A published to YouTube and GitHub Release and integrated into both READMEs; public playback and download links verified; public use of the local synthesized narration authorized; automated media, integrity, and visual checks passed; human audible listening review remains not run
...
### 13.5 README, YouTube, and GitHub Release handoff

The user selected option A and authorized public publication of the existing narration on 2026-09-15. The GitHub Release `v0.2.1` asset remains the durable download backup. On 2026-09-15 the user additionally approved a public YouTube upload and chose the YouTube watch page as the primary README playback destination.
...
2. Keep `haas-explainer-bilingual.mp4` as a public download asset on `https://github.com/superops-team/haas/releases/tag/v0.2.1`. The user uploaded the reviewed film to the authorized YouTube channel as video `rmMq4Dpu-js`; anonymous watch-page and oEmbed requests succeeded.
3. Link the single local README cover to the canonical `https://www.youtube.com/watch?v=rmMq4Dpu-js` watch URL. Also retain a clearly labeled GitHub Release download link. Remove only the homepage GIF embeds: retain the six GIF files for history and deep links, and retain the detailed static and interactive architecture destinations.
...
5. GitHub README does not provide a reliable inline video player. The cover opens the public YouTube watch page where playback is available; do not use an iframe, autoplay, JavaScript, tracking, or wording that claims playback is embedded inside GitHub README.
6. Publish bilingual title and description plus these factual YouTube chapter anchors: 00:00, 00:20, 00:38, 01:04, 01:30, 01:54, and 02:28. Upload the reviewed cover as the custom thumbnail only when the channel permits it; otherwise keep YouTube's generated thumbnail and retain the local README cover.
...
6. Retain the verified MP4 on Release `v0.2.1`, then upload the same file to YouTube using the user's authorized channel. Verify YouTube reports processing complete, visibility is public, duration is approximately 2:57, and an unauthenticated request resolves the watch page and public player metadata. Record the returned video ID; never construct or guess it.
...
8. Update both READMEs only after step 6 passes. Verify the local image path, the YouTube watch click target, the explicit GitHub Release download fallback, removal of all six homepage GIF embeds, preservation of detailed static/interactive links, and factual bilingual equivalence. Render or inspect the resulting GitHub Markdown at desktop and narrow widths.
